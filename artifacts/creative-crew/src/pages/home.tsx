...
import { Clapperboard, Film, Video, Sparkles, Loader2, ArrowRight } from "lucide-react";
          ...
          {data && !isPending && (
            <div className="flex-1 space-y-16 animate-in fade-in zoom-in-95 duration-700 pb-12 mt-8">
              <div className="space-y-6">
                ...
                <h1 className="text-5xl md:text-6xl font-serif font-bold leading-tight tracking-tight" data-testid="text-title">
                  {data.title}
                </h1>
                <p className="text-xl md:text-2xl text-muted-foreground leading-relaxed font-serif italic border-l-4 border-primary pl-6 py-2" data-testid="text-logline">
                  {data.logline}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-4">
                  <h3 className="text-sm font-mono tracking-widest uppercase text-muted-foreground">Central Idea</h3>
                  <p className="text-lg leading-relaxed text-foreground/90" data-testid="text-central-idea">{data.centralIdea}</p>
                </div>
                <div className="space-y-4">
                  <h3 className="text-sm font-mono tracking-widest uppercase text-muted-foreground">Narrative Approach</h3>
                  <p className="text-lg leading-relaxed text-foreground/90" data-testid="text-narrative-approach">{data.narrativeApproach}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-6">
                  <div className="space-y-4">
                    <h3 className="text-sm font-mono tracking-widest uppercase text-muted-foreground">Emotional Direction</h3>
                    <p className="text-foreground/90 leading-relaxed" data-testid="text-emotional-direction">{data.emotionalDirection}</p>
                  </div>
                  <div className="space-y-4">
                    <h3 className="text-sm font-mono tracking-widest uppercase text-muted-foreground">Tone</h3>
                    <div className="flex flex-wrap gap-2" data-testid="list-tone">
                      {data.tone.map((t, i) => (
                        <Badge key={i} variant="secondary" className="bg-secondary/50 font-mono text-xs hover:bg-secondary/80 transition-colors cursor-default">{t}</Badge>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="space-y-4">
                    <h3 className="text-sm font-mono tracking-widest uppercase text-muted-foreground">Audience Promise</h3>
                    <p className="text-foreground/90 leading-relaxed" data-testid="text-audience-promise">{data.audiencePromise}</p>
                  </div>
                  <div className="space-y-4">
                    <h3 className="text-sm font-mono tracking-widest uppercase text-muted-foreground">Guardrails</h3>
                    <ul className="space-y-3" data-testid="list-guardrails">
              ...
              <div className="space-y-6 bg-secondary/10 p-8 rounded-xl border border-border/50 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-1 h-full bg-primary" />
                <h3 className="text-sm font-mono tracking-widest uppercase text-muted-foreground">Visual Principles</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6" data-testid="list-visual-principles">
                  {data.visualPrinciples.map((vp, i) => (
                    <div key={i} className="flex gap-3 items-start">
                      <div className="w-1.5 h-1.5 rounded-full bg-primary mt-2 shrink-0 shadow-[0_0_8px_rgba(255,0,0,0.8)]" />
                      <p className="text-sm leading-relaxed text-foreground/90">{vp}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex justify-between items-center pt-8 border-t border-border/30">
                <div className="w-16 h-1 bg-border/50" />
                <div className="text-xs font-mono text-muted-foreground flex items-center gap-2 bg-secondary/20 px-3 py-1.5 rounded-full">
                  Synthesized by <span className="text-foreground font-semibold">{data.generatedBy}</span>
                  <span className="w-1.5 h-1.5 rounded-full bg-primary/80 animate-pulse shadow-[0_0_5px_rgba(255,0,0,0.5)]" />
                </div>
              </div>
            </div>
          )}
