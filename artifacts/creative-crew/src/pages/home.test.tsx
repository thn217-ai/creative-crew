...
const progress = () => screen.getByRole("status", { name: "Treatment progress" });
...
test("loading a selected saved treatment shows progress rather than an empty or approved result", async (t) => {
  ...
  const loading = await screen.findByRole("status", { name: "Treatment progress" });
  assert.match(loading.textContent ?? "", /Synthesizing Vision/);
  assert.match(loading.textContent ?? "", /Google ADK \/ Gemini/);
  assert.equal(screen.queryByText("TREATMENT APPROVED"), null);
  ...
  assert.equal(screen.queryByRole("status", { name: "Treatment progress" }), null);
});
...
test("generation loading hides the prior result, keeps the brief, and blocks duplicate submissions", async (t) => {
  ...
  const processing = await screen.findByRole<HTMLButtonElement>("button", { name: "Processing" });
  assert.equal(processing.disabled, true);
  assert.match(progress().textContent ?? "", /Synthesizing Vision/);
  ...
  await act(async () => pending.resolve(json(project)));
  await screen.findByText("TREATMENT APPROVED");
  await ready();
  assert.equal(screen.queryByRole("status", { name: "Treatment progress" }), null);
});
...
const failures = [
  ...
  },
  {
    ...
    message: "Failed to generate treatment.",
...
for (const failure of failures) {
  test(`${failure.name} shows useful details and the same brief can be retried successfully`, async (t) => {
    ...
    assert.equal(screen.queryByRole("status", { name: "Treatment progress" }), null);
    assert.equal(screen.queryByText("TREATMENT APPROVED"), null);
    assert.equal(submitButton().disabled, false, "A provider failure must not trap the form in Processing");
